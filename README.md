...
- 평소 영화를 즐겨보지 않아 사용자 입장에서 생각하는 것이 어려웠다. 개발 기간이 넉넉했다면 주변 사람들의 피드백을 받으면 좋았을 것 같다.
- 대부분의 데이터를 서버에서 가공하고 클라이언트는 출력만 해주는 식으로 진행하였더니, 자바스크립트로도 충분히 가공할 수 있는 데이터를 매번 서버에 요청하게 되어 반응속도가 많이 느리다. 실사용 측면에서 고려해야할 부분을 하나 더 배울 수 있었다. 다음 개발 시에는 이런 부분을 유의해야겠다.
